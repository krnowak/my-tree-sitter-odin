...
module.exports = grammar({
  ...
  externals : $ => [
    ...
    $._a_fast_math, // bitset of Fast_Math_Flags
    $._a_fini, // nothing
    ...
    $._t_xor,
    $._t_xoreq,
    $._cc_any_cc, // mostly for checking if CC is an expected symbol
    $._cc_odin,
    $._cc_contextless,
    $._cc_cdecl,
    $._cc_stdcall,
  ...
  supertypes : $ => [
    ...
    $.statement,
    $.expression,
    $.rhs_expression, // TODO: drop from supertypes, eventually
    $.lhs_expression, // TODO: drop from supertypes, eventually
    $.rhs_expression_cc, // TODO: drop from supertypes, eventually
    $.lhs_expression_cc, // TODO: drop from supertypes, eventually
    $.rhs_expression_cc_nie, // TODO: drop from supertypes, eventually
    $.lhs_expression_cc_nie, // TODO: drop from supertypes, eventually
    $.atom_expression,
    $.atom_expression_nlve, // TODO: drop from supertypes, eventually
    // TODO: add unary_expression, eventually
    $.rhs_unary_expression, // TODO: drop from supertypes, eventually
    $.lhs_unary_expression, // TODO: drop from supertypes, eventually
    $.rhs_unary_expression_cc, // TODO: drop from supertypes, eventually
    $.lhs_unary_expression_cc, // TODO: drop from supertypes, eventually
  ],
  ...
  conflicts : $ => [
    ...
    //    Possible interpretations:
    //
    //      1:  package  _kw_proc  _t_openparen  (_identifier_or_poly_type_field  identifier)  •  _t_comma  …
    //      2:  package  _kw_proc  _t_openparen  (type  identifier)  •  _t_comma  …
    //
    [$.type, $._identifier_or_poly_type_field],
    //    Unresolved conflict for symbol sequence:
    //
  ...
  precedences : $ => [
    ...
    //    Possible interpretations:
    //
    //      1:  package  _t_hash  _d_force_inline  _t_openparen  (_common_atom_subexpressions  call_atom_expression)  •  _kw_or_break  …
    ...
    //
    // call_atom_expression -> _common_atom_subexpressions ->
  ...
  rules : {
    ...
    _assign_statement_nfs : $ => seq(
      alias($._lhs_expression_list, $.lhs_expressions),
      alias($._assign_operator, $.op),
      alias($._rhs_expression_list, $.rhs_expressions),
    ),

    _assign_statement_nfs_cc : $ => seq(
      alias($._lhs_expression_cc_list, $.lhs_expressions),
      alias($._assign_operator, $.op),
      alias($._rhs_expression_cc_list, $.rhs_expressions),
    ),

    _assign_statement_nfs_cc_nie : $ => seq(
      alias($._lhs_expression_cc_nie_list, $.lhs_expressions),
      alias($._assign_operator, $.op),
      alias($._rhs_expression_cc_nie_list, $.rhs_expressions),
    ),
    ...
    _const_declaration_nfs : $ => seq(
      alias($._identifier_list, $.names),
      $._t_colon,
      optional($.type),
      $._t_colon,
      alias($._rhs_expression_list, $.values)
    ),
    ...
    _const_declaration_nfs_cc : $ => seq(
      ...
      optional($.type),
      $._t_colon,
      alias($._rhs_expression_cc_list, $.values)
    ),
    ...
    statement_directive_statement : $ => seq(
      alias($._statement_directive, $.directive),
      choice(
        $.block_statement,
        $.if_statement,
        $.when_statement,
    ...
    _statement_directive : $ => seq(
      $._t_hash,
      alias($._statement_directive_name, $.directive_name),
    ),

    _statement_directive_name : $ => choice(
      $._d_bounds_check,
      $._d_no_bounds_check,
      $._d_no_type_assert,
      $._d_type_assert,
    ...
    _unroll_directive : $ => choice(
      alias($._basic_unroll_directive, $.directive),
      alias($._unroll_directive_with_args, $.directive_call),
    ),

    _basic_unroll_directive : $ => seq(
      $._t_hash,
      alias($._d_unroll, $.directive_name),
    ),

    _unroll_directive_with_args : $ => seq(
      $._t_hash,
      alias($._d_unroll, $.directive_name),
      $._t_openparen,
      $._unroll_parameter_list,
      $._t_closeparen,
    ),

    _unroll_parameter_list : $ => repeatComma1Trailing($, $.unroll_parameter),
    ...
    _partial_directive : $ => seq(
      $._t_hash,
      alias($._d_partial, $.directive_name),
    ),

    reverse_for_directive_statement : $ => seq(
      alias($._reverse_directive, $.directive),
      choice(
    ...
    _reverse_directive : $ => seq(
      $._t_hash,
      alias($._d_reverse, $.directive_name),
    ),
    ...
    inline_directive : $ => seq(
      alias($._inline_directive, $.directive),
      $.inline_directive_expression,
    ),

    _inline_directive : $ => seq(
      $._t_hash,
      alias($._inline_directive_name, $.directive_name),
    ),

    _inline_directive_name : $ => choice(
      $._d_force_inline,
      $._d_force_no_inline,
    ),
    ...
    must_tail_directive : $ => seq(
      alias($._must_tail_directive, $.directive),
      $.must_tail_directive_expression,
    ),

    _must_tail_directive : $ => seq(
      $._t_hash,
      alias($._d_must_tail, $.directive_name),
    ),
    ...
    for_statement : $ => seq(
      ...
      seq(
        ...
        choice(
          seq(
            // TODO: expr_level -1
            optional(alias($.simple_statement_nfs_cc_nie, $.for_cond)),
            optional($.for_post),
          ),
          // TODO: expr_level -1
          alias($.in_statement_nfs_cc_nie, $.for_range),
        )
      ),
      $._typical_block,
    ),

    _typical_block : $ => choice(
      $.block_statement,
      $.do_statement,
    ),
    ...
    // dummy, unused, mostly so it exists for supertypes, so I can
    // also use expression as alias
    expression : $ => seq(),

    lhs_expression : $ => choice(
      $.lhs_unary_expression,
      alias($.lhs_binary_expression, $.binary_expression),
      alias($.lhs_ternary_expression, $.ternary_expression),
    ),

    rhs_expression : $ => choice(
      $.rhs_unary_expression,
      alias($.rhs_binary_expression, $.binary_expression),
      alias($.rhs_ternary_expression, $.ternary_expression),
    ),

    lhs_expression_cc : $ => choice(
      $.lhs_unary_expression_cc,
      alias($.lhs_binary_expression_cc, $.binary_expression),
      alias($.lhs_ternary_expression_cc, $.ternary_expression),
    ),

    rhs_expression_cc : $ => choice(
      $.rhs_unary_expression_cc,
      alias($.rhs_binary_expression_cc, $.binary_expression),
      alias($.rhs_ternary_expression_cc, $.ternary_expression),
    ),

    lhs_expression_cc_nie : $ => choice(
      $.lhs_unary_expression_cc,
      alias($.lhs_binary_expression_cc_nie, $.binary_expression),
      alias($.lhs_ternary_expression_cc_nie, $.ternary_expression),
    ),

    rhs_expression_cc_nie : $ => choice(
      $.rhs_unary_expression_cc,
      alias($.rhs_binary_expression_cc_nie, $.binary_expression),
      alias($.rhs_ternary_expression_cc_nie, $.ternary_expression),
    ),
    ...
    lhs_unary_expression : $ => prec.right('unary', choice(
      alias($.lhs_cast_expression, $.cast_expression),
      alias($.lhs_transmute_expression, $.transmute_expression),
      alias($.lhs_auto_cast_expression, $.auto_cast_expression),
      alias($.lhs_prefix_unary_expression, $.unary_expression),
      $.implicit_selector_expression,
      $.atom_expression,
    )),

    rhs_unary_expression : $ => prec.right('unary', choice(
      alias($.rhs_cast_expression, $.cast_expression),
      alias($.rhs_transmute_expression, $.transmute_expression),
      alias($.rhs_auto_cast_expression, $.auto_cast_expression),
      alias($.rhs_prefix_unary_expression, $.unary_expression),
      $.implicit_selector_expression,
      $.atom_expression,
      $.literal_value,
    ...
    lhs_unary_expression_cc : $ => prec.right('unary', choice(
      alias($.lhs_cast_expression_cc, $.cast_expression),
      alias($.lhs_transmute_expression_cc, $.transmute_expression),
      alias($.lhs_auto_cast_expression_cc, $.auto_cast_expression),
      alias($.lhs_prefix_unary_expression_cc, $.unary_expression),
      $.implicit_selector_expression,
      $.atom_expression_nlve,
    )),

    rhs_unary_expression_cc : $ => prec.right('unary', choice(
      alias($.rhs_cast_expression_cc, $.cast_expression),
      alias($.rhs_transmute_expression_cc, $.transmute_expression),
      alias($.rhs_auto_cast_expression_cc, $.auto_cast_expression),
      alias($.rhs_prefix_unary_expression_cc, $.unary_expression),
      $.implicit_selector_expression,
      $.atom_expression_nlve,
      $.literal_value,
    ...
    lhs_prefix_unary_expression : $ => seq(
      alias($._unary_prefix_operator, $.op),
      $.lhs_unary_expression,
    ),
    ...
    atom_expression_nlve : $ => choice(
      $.operand,
      alias($.call_atom_expression_nlve, $.call_atom_expression),
      alias($.selector_atom_expression_nlve, $.selector_atom_expression),
      alias($.type_assertion_atom_expression_nlve, $.type_assertion_atom_expression),
      alias($.type_inference_atom_expression_nlve, $.type_inference_atom_expression),
      alias($.slice_index_atom_expression_nlve, $.slice_index_atom_expression),
      alias($.matrix_index_atom_expression_nlve, $.matrix_index_atom_expression),
      alias($.slice_atom_expression_nlve, $.slice_atom_expression),
      alias($.dereference_atom_expression_nlve, $.dereference_atom_expression),
      alias($.or_return_atom_expression_nlve, $.or_return_atom_expression),
      alias($.or_continue_atom_expression_nlve, $.or_continue_atom_expression),
      alias($.or_break_atom_expression_nlve, $.or_break_atom_expression),
      alias($.inline_directive_nlve, $.inline_directive),
      alias($.must_tail_directive_nlve, $.must_tail_directive),
    ),
    ...
    directive_with_type : $ => seq(
      alias($._type_directive, $.directive),
      $._type_maybe_in_parentheses,
    ),

    _type_directive : $ => seq(
      $._t_hash,
      alias($._type_directive_name, $.directive_name),
    ),

    _type_directive_name : $ => choice(
      $._d_type,
      $._d_column_major,
      $._d_row_major,
      $._d_simd,
    ...
    just_directive : $ => alias($._just_directive, $.directive),

    _just_directive : $ => seq(
      $._t_hash,
      alias($._no_arg_directive_name, $.directive_name),
    ),

    _no_arg_directive_name : $ => choice(
      $._d_file,
      $._d_directory,
      $._d_line,
      $._d_procedure,
    ...
    directive_call : $ => seq(
      alias($._call_directive, $.directive),
      $._call_paren_part,
    ),

    _call_directive : $ => seq(
      $._t_hash,
      alias($._call_directive_name, $.directive_name),
    ),

    _call_directive_name : $ => choice(
      $._d_location,
      $._d_config,
      $._d_defined,
      $._d_exists,
    ...
    procedure_literal : $ => seq(
      ...
      repeat(alias($._procedure_trailing_directive, $.directive)),
      //optional($._implicit_semicolon),
      choice(
        $._t_uninit,
        $.block_statement,
    ...
    // TODO: move to operator helpers
    _procedure_trailing_directive : $ => seq(
      $._t_hash,
      alias($._procedure_trailing_directive_name, $.directive_name),
    ),

    _procedure_trailing_directive_name : $ => choice(
      $._d_optional_ok,
    ...
    _procedure_parameters : $ => seq(
      $._t_openparen,
      optional(alias($._procedure_field_list, $.field_list)),
      $._t_closeparen,
    ),
    ...
    calling_convention : $ => choice(
      ...
      $._cc_preserve_most,
      $._cc_preserve_all,
      alias($.string, $.bad_calling_convention),
    ),

    // TODO: move to operator helpers
    _procedure_field_list : $ => choice(
      $._type_procedure_field_list,
      $._name_procedure_field_list,
    ),

    // TODO: move to operator helpers
    _type_procedure_field_list : $ => repeatComma1Trailing($, alias($._type_procedure_field, $.field)),

    _name_procedure_field_list : $ => repeatComma1Trailing($, alias($._procedure_parameter_pack, $.field)),

    _type_procedure_field : $ => seq(
      repeat($.field_prefix),
    ...
    _procedure_parameter_pack : $ => seq(
      alias($._identifier_or_poly_type_list, $.names),
      $._t_colon,
      // Variadic type is normally only acceptable for the last
    ...
    // TODO: move to operator helpers
    procedure_results : $ => seq(
      $._t_arrowright,
      choice(
        alias($._t_not, $.diverging),
        alias($._procedure_results_single_type, $.single_type),
        alias($._procedure_results_multiple, $.multiple),
      ),
    ),

    _procedure_results_single_type : $ => $.type,

    _procedure_results_multiple : $ => seq(
      $._t_openparen,
      optional(alias($._result_field_list, $.field_list)),
      $._t_closeparen,
    ),
    ...
    _result_field_list : $ => choice(
      // type, type, type, …
      $._type_result_field_list,
      // name, name, … : type, name, name, … : type
      //
      // with default values too
      $._name_result_field_list,
    ),

    _type_result_field_list : $ => repeatComma1Trailing($, alias($._type_result_field, $.field)),

    _type_result_field : $ => seq(
      repeat($.field_prefix),
      $._type_maybe_in_parentheses,
    ),

    _name_result_field_list : $ => repeatComma1Trailing($, alias($._result_single_pack, $.field)),
    ...
    _slicelike_type : $ => choice(
      $.slice_type,
      $.fixed_array_type,
      $.inferred_size_fixed_array_type,
      $.dynamic_array_type,
    ...
    inferred_size_fixed_array_type : $ => seq(
      $._t_openbracket,
      $._t_question,
      $._t_closebracket,
      $._type_maybe_in_parentheses,
    ...
    fixed_capacity_dynamic_array_type : $ => seq(
      $._t_openbracket,
      $._kw_dynamic,
      $._t_semicolon,
      $.rhs_expression,
    ...
    struct_type : $ => seq(
      $._kw_struct,
      optional(alias($.struct_union_polyparams, $.poly_params)),
      optional($.struct_directives),
      //optional($._implicit_semicolon),
      optional($.where_clauses),
      //optional($._implicit_semicolon),
      $._t_openbrace,
      optional(alias($._struct_field_list, $.field_list)),
      $._t_closebrace,
    ),

    struct_directives : $ => repeat1($._struct_directive),

    _struct_directive : $ => choice(
      alias($._basic_struct_directive, $.directive),
      alias($._struct_directive_call, $.directive_call),
    ),

    // TODO: move to operator helpers
    _basic_struct_directive : $ => seq(
      $._t_hash,
      alias($._no_arg_struct_directive_name, $.directive_name),
    ),

    // TODO: move to operator helpers
    _struct_directive_call : $ => seq(
      $._t_hash,
      alias($._struct_directive_name, $.directive_name),
      // TODO: expr_level -1
      '(',
      $.rhs_expression_cc,
      ')',
    ...
    _no_arg_struct_directive_name : $ => choice(
      $._d_all_or_none,
      $._d_packed,
      $._d_raw_union,
      $._d_simple,
    ...
    // TODO: move to operator helpers
    _struct_field_list : $ => choice(
      $._type_only_struct_field_list,
      $._named_struct_field_list,
    ),

    _type_only_struct_field_list : $ => repeatComma1Trailing($, seq(
      alias($._type_only_struct_field, $.field),
    )),

    _type_only_struct_field : $ => seq(
      repeat($.field_prefix),
      $._type_maybe_in_parentheses,
    ),

    // TODO: move to operator helpers
    _named_struct_field_list : $ => repeatComma1Trailing($, seq(
      alias($._named_struct_field, $.field),
    )),

    _named_struct_field : $ => seq(
      repeat($.field_prefix),
      alias($._identifier_list, $.names),
      $._t_colon,
      $._type_maybe_in_parentheses,
    ...
    struct_union_polyparams : $ => seq(
      $._t_openparen,
      alias($._struct_union_polyparam_field_list, $.field_list),
      $._t_closeparen,
    ),
    ...
    //
    // TODO: move to operator helpers
    _struct_union_polyparam_field_list : $ => repeatComma1Trailing($, $._struct_union_polyparam_field_alias),

    // TODO: move to operator helpers
    _struct_union_polyparam_field_alias : $ => alias($._struct_union_polyparam_field, $.field),

    // TODO: move to operator helpers
    _struct_union_polyparam_field : $ => seq(
      alias($._identifier_or_poly_type_list, $.names),
      $._t_colon,
      optional($._type_maybe_in_parentheses),
      optional(seq(
    ...
    union_directives : $ => repeat1($._union_directive),

    _union_directive : $ => choice(
      alias($._basic_union_directive, $.directive),
      alias($._union_directive_call, $.directive_call),
    ),

    // TODO: move to operator helpers
    _basic_union_directive : $ => seq(
      $._t_hash,
      alias($._no_arg_union_directive_name, $.directive_name),
    ),

    // TODO: move to operator helpers
    _union_directive_call : $ => seq(
      $._t_hash,
      alias($._d_align, $.directive_name),
      $.parentheses_expression,
    ),

    _no_arg_union_directive_name : $ => choice(
      $._d_no_nil,
      $._d_shared_nil,
    ),
    ...
    inline_asm : $ => seq(
      ...
      optional($.inline_asm_directives),
      //optional($._implicit_semicolon),
      $._t_openbrace,
    ...
    inline_asm_directives : $ => repeat1(alias($._inline_asm_directive, $.directive)),

    // TODO: move to operator helpers
    _inline_asm_directive : $ => seq(
      $._t_hash,
      alias($._inline_asm_directive_name, $.directive_name),
    ),

    _inline_asm_directive_name : $ => choice(
      $._d_align_stack,
      $._d_att,
      $._d_intel,
      $._d_side_effects,
    ...
    _common_atom_subexpressions : $ => prec('atom_expression', choice(
      // operands
      $.identifier,
      $.parentheses_expression,
      $.just_directive,
    ...
    _common_atom_subexpressions_nlve : $ => choice(
      // operands
      $.identifier,
      $.parentheses_expression,
      $.just_directive,
    ...
    _slice_part : $ => seq(
      $._t_openbracket,
      // TODO: allow_range = false
      optional(alias($.rhs_expression, $.low_bound)),
      $._t_colon,
      // TODO: allow_range = false
      optional(alias($.rhs_expression, $.high_bound)),
      $._t_closebracket,
    ),
    ...
    field_prefix : $ => choice(
      ...
      alias($._field_prefix_directive, $.directive),
    ),

    _field_prefix_directive : $ => seq(
      $._t_hash,
      alias($._field_prefix_directive_name, $.directive_name),
    ),

    _field_prefix_directive_name : $ => choice(
      $._d_by_ptr,
      $._d_any_int,
      $._d_c_vararg,
      $._d_const,
    ...
    literal_value : $ => seq(
      ...
      // TODO: expr_level = 0
      optional(choice(
        $._value_list_trailing,
        $._key_value_list_trailing,
      )),
      $._t_closebrace,
    ),

    _value_list_trailing : $ => repeatComma1Trailing($, $._value),

    _key_value_list_trailing : $ => repeatComma1Trailing($, $.key_value)
,
    key_value : $ => seq(
      $._value,
      $._t_eq,
    ...
    _identifier_or_poly_type_list : $ => repeatComma1($, $._identifier_or_poly_type_field),

    _identifier_or_poly_type_field : $ => seq(
      repeat($.field_prefix),
      choice(
        $.identifier,
        $.poly_type,
