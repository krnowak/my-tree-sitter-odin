...
#define ODIN_DIRECTIVE_SYMBOLS \
  ...
  ODIN_SYMBOL(KWAD_PRESERVE), \
  ODIN_SYMBOL(KWAD_PROCEDURE), \
  ODIN_SYMBOL(KWAD_PURE), \
...
// other terminals that do not add a semicolon
#define ODIN_CALLING_CONVENTIONS \
  ODIN_SYMBOL(KWAD_CC_ANY_CC), \
  ODIN_SYMBOL(KWAD_CC_ODIN), \
  ODIN_SYMBOL(KWAD_CC_CONTEXTLESS), \
  ODIN_SYMBOL(KWAD_CC_CDECL), \
  ODIN_SYMBOL(KWAD_CC_STDCALL), \
  ODIN_SYMBOL(KWAD_CC_FASTCALL), \
  ODIN_SYMBOL(KWAD_CC_NONE), \
  ODIN_SYMBOL(KWAD_CC_NAKED), \
  ODIN_SYMBOL(KWAD_CC_INLINEASM), \
  ODIN_SYMBOL(KWAD_CC_WIN64), \
  ODIN_SYMBOL(KWAD_CC_SYSV), \
  ODIN_SYMBOL(KWAD_CC_PRESERVE_NONE), \
  ODIN_SYMBOL(KWAD_CC_PRESERVE_MOST), \
  ODIN_SYMBOL(KWAD_CC_PRESERVE_ALL), \
  ODIN_SYMBOL(KWAD_CC_SYSTEM)
...
// matches [a-z0-9/]
#define ODIN_CC_SET_LETTERS_LEN (3)
static const TSCharacterRange odin_cc_set_letters[] = {
  {'/', '/'}, {'0', '9'}, {'a', 'z'},
};
...
static inline bool
odin_is_cc_char(TSLexer *lexer) {
  return set_contains(odin_cc_set_letters, ODIN_CC_SET_LETTERS_LEN, lexer->lookahead);
}
...
#define ODIN_CASE_STRING_QUOTE_CASE(c, sym, sym2) \
  ...
  // ODIN_MUL(*): ODIN_MULMUL(**), ODIN_MULEQ(*=)
  case ODIN_CASE_MUL:
    odin_consume(lexer);
    if (!odin_eof(lexer)) {
      switch (lexer->lookahead) {
      case '*':
        odin_consume(lexer);
        ODIN_RET_SYMBOL(lexer, ODIN_MULMUL);
      case '=':
        odin_consume(lexer);
        ODIN_RET_SYMBOL(lexer, ODIN_MULEQ);
      }
    }
    ODIN_RET_SYMBOL(lexer, ODIN_MUL);
